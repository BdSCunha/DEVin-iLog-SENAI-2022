...
public class UsuarioService implements UserDetailsService {
    ...
    private String segredo = "bB2?lP0%oV7<dM3.pS8%oS8<sE6&iK2}"; // chave de assinatura

    public Usuario criar(Usuario usuario) {
        boolean existente = repo.existsUsuarioByEmail(usuario.getEmail()); //true ou false
        if (existente) throw new RegistroExistenteException("Usuario", usuario.getEmail());
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        if (usuarioOpt.isEmpty()) throw new UsernameNotFoundException("Usuário não encontrado!");
